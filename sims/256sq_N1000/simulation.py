import sys
sys.path.append('/home/corvus/docs/uni/bachelor_thesis/code/Imazalil/')
...
with open("/home/corvus/docs/uni/bachelor_thesis/code/Imazalil/actor-critic/rcParams.yml", 'r') as pars:
    params = yaml.load(pars)
...
if(__name__ == '__main__'):
    ...
    fig, ax = grid.plot(densities=[npred, nprey], currenttimestep=-1, timesteps=ts, dpi=DPI,
    ...
    for _ in range(ts):
        ...
        npred.append(grid.get_num_pred())
        if(stepcnt >= ff):
            fig, ax = grid.plot(densities=[npred, nprey], currenttimestep=_, timesteps=ts,
